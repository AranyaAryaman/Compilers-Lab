...
enum Type Coercible(Operator *op, enum Type t1, enum Type t2)
{
  ...
  if (oper == LT || oper == GT || oper == EQ_OP || oper == NE_OP)
  {
    ...
    {
    case T_INT:
      if (t2 == T_FLOAT || t2 == T_INT)
        return T_BOOL;
    case T_FLOAT:
      if (t2 == T_FLOAT || t2 == T_INT)
        return T_BOOL;
    default:
      IncompatibleOperands(op, t1, t2);
      return T_ERROR;
    }
  }
  ...
  case T_ERROR:
    ...
    return T_ERROR;
  default:
    Formatted(NULL, "CodeGen: Unknown Type: %d", t1);
  }
  IncompatibleOperands(op, t1, t2);
  return T_ERROR;
}
...
enum Type Coercible(Operator *op, enum Type t)
{
  ...
  case T_ERROR:
    ...
    return T_ERROR;
  default:
    Formatted(NULL, "CodeGen: Unknown Type: %d", t);
  }
  IncompatibleOperands(op, t);
  return T_ERROR;
}
